...
const activeURLKeywords = computed(() => {
    return Keywords.value.filter(e => e.url === STRUCTURE.value[ACTIVE_URL.value])
})
...
const Transactional = computed(() => {
    if (INPUT_SEARCH.value !== '') {
        ...
        return Keywords.value.filter((e: IKeyword) => e.type === 'Transactional' && e.keyword.includes(INPUT_SEARCH.value) && e.url === '')
    }
    return Keywords.value.filter((e: IKeyword) => e.type === 'Transactional' && e.url === '')
})

/**
 * Enable / disable button if inptu_url has content or not
 */
const disableButtonAddURL = computed(() => {
    return INPUT_URL.value.length < 4
})
...
const amountURL = computed(() => activeURLKeywords.value.reduce((accumulator: number, element: IKeyword) => accumulator + element.volume, 0))
...
onMounted(() => {
    ...
    if (STRUCTURE.value.length > 0) ACTIVE_URL.value = 0
    INPUT_ACTIVE_URL.value = STRUCTURE.value[ACTIVE_URL.value]
})
...
<template>
    <NuxtLayout name="projects">
        <section class="flex justify-around">
            <div class="flex justify-center gap-4 items-center">
                ...
                <input type="text" v-model="INPUT_URL"
                    ...
                    placeholder="example.com or /content">
                <button type="button"
                    ...
                    :disabled="disableButtonAddURL" @click="onClickButtonAddURL">Add {{ STRUCTURE.length === 0 ? 'Root '
                        : '' }}URL</button>
            </div>
            <button type="button"
                ...
                :disabled="LAST_ACTION.length === 0" @click="onClickButtonUndoLastAction">Undo last action</button>
        </section>
        <div class="grid grid-cols-3 gap-4 1/2">
            ...
            <article class="flex flex-col gap-2 h-full">
                ...
                <ul class="flex flex-col max-h-screen gap-2 overflow-y-auto">
                    <li v-for="item in STRUCTURE" @click="onClickURL(item)"
                        ...
                        :class="STRUCTURE[ACTIVE_URL] === item ? 'dark:bg-blue-400 bg-blue-600 text-blue-700' : ' bg-blue-800  dark:bg-blue-800'">
            ...
            <article class="flex flex-col gap-2 h-full">
                <h2 class="block text-center text-xl mb-4">Keywords of {{ STRUCTURE[ACTIVE_URL] }}</h2>
                <section class="flex justify-between items-center gap-2">
                    <input type="text" v-model="INPUT_ACTIVE_URL"
                        @input="onInputTextURL"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <button type="button"
                        ...
                        :disabled="ACTIVE_URL === 0"
                        @click="onClickButtonRemoveURL()"> RemoveURL
                    </button>
                </section>
                <section class="flex justify-between items-center gap-2">
                    <button type="button"
                        class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700"
                        @click="onClickButtonAddKeywordsToURL()"> Add Keywords to URL
                    </button>
